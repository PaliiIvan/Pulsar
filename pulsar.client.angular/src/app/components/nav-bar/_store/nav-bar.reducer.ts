...
const navBarReducer = createReducer<NavBarState>(
    initialState,
    on(NavBarActions.authProcessStarted, (state) => ({
        ...state,
        isAuthProcess: true,
    })),
    on(NavBarActions.authProcessFinished, (state) => ({
        ...state,
        isAuthProcess: false,
    })),
    on(NavBarActions.streamInitStarted, (state) => ({
        ...state,
        isStreaminitProcess: true,
    })),
    on(NavBarActions.streamInitFinished, (state) => ({
        ...state,
        isStreaminitProcess: false,
    }))
);
